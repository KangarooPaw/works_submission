...
bool CInputMouse::GetMousePressLeft(void)
{
	...
	/* 今のマウスの状態 */
	bool current = m_MState.lButton;
	/* 前の状態がtrueで、今の状態がtrueならば、クリックしていると判定する */
	bool ret = current && prevState[0];
	/* 今の状態を保存する */
	prevState[0] = current;
	/* 判定結果を返す */
...
bool CInputMouse::GetMouseReleaseLeft(void)
{
	/* 前に取得した時のマウスの状態 */
	static bool prevState[sizeof(m_dIMouseState.rgbButtons) / sizeof(m_dIMouseState.rgbButtons[0])];
	/* 今のマウスの状態 */
	bool current = m_MState.lButton;
	/* 前の状態がtrueで、今の状態がfalseならば、クリックを離したと判定する */
	bool ret = !current && prevState[0];
	/* 今の状態を保存する */
	prevState[0] = current;
	/* 判定結果を返す */
...
bool CInputMouse::GetMouseReleaseRight(void)
{
	/* 前に取得した時のマウスの状態 */
	static bool prevState[sizeof(m_dIMouseState.rgbButtons) / sizeof(m_dIMouseState.rgbButtons[0])];
	/* 今のマウスの状態 */
	bool current = m_MState.rButton;
	/* 前の状態がfalseで、今の状態がtrueならば、クリックを離したと判定する */
	bool ret = !current && prevState[0];
	/* 今の状態を保存する */
	prevState[0] = current;
	/* 判定結果を返す */
